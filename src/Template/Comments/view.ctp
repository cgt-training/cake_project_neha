...
<div class="comments view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        <tr>
            ...
            <td><?= $comment->has('comment') ? $this->Html->link($comment->comment->comment_id, ['controller' => 'Comments', 'action' => 'view', $comment->comment->comment_id]) : '' ?></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><?= h($comment->modified_date) ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Comment Text') ?></h4>
        <?= $this->Text->autoParagraph(h($comment->comment_text)); ?>
    </div>
</div>
